  ...
  class Session {
    ...
    save() {
      ...
      return getAdapter().upsert(this.id, payload,
        provider.get('cookie opts long').maxAge / 1000);
    }
    ...
    static get(ctx) {
      ...
      const sessionId = ctx.cookies.get(provider.cookieName('session'), {
        signed: provider.get('cookie opts long').signed,
      }) || uuid();

      return this.find(sessionId).then((session) => {
        // refresh the session duration
        ctx.cookies.set(provider.cookieName('session'), sessionId, provider.get('cookie opts long'));
        return session;
      });
    }
  }
