...
module.exports = provider => async function decodeRequest(ctx, next) {
  ...
  if (provider.enabled('features.encryption') && params.request.split('.').length === 5) {
    try {
      const header = JWT.header(params.request);

      assert(provider.get('requestObjectEncryptionAlgValues').indexOf(header.alg) !== -1,
        'unsupported encrypted request alg');
      assert(provider.get('requestObjectEncryptionEncValues').indexOf(header.enc) !== -1,
  ...
  if (client.requestObjectSigningAlg) {
    ...
    ctx.assert(provider.get('requestObjectSigningAlgValues').indexOf(alg) !== -1, 400,
