 ...
 */
module.exports = provider => function throwNotSupported(ctx, next) {
  const params = ctx.oidc.params;

  if (provider.disabled('request') && params.request !== undefined) {
    ctx.throw(400, 'request_not_supported', {
      error_description: 'request parameter provided but not supported',
    });
  }

  if (provider.disabled('requestUri') && params.request_uri !== undefined) {
